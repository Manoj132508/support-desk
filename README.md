...
Requires Node 22+ and Python 3.12+. Signing in and holding a conversation need a database (set
`MONGODB_URI`); without one the client, API and AI service all still start, and the routes later
phases build return `501` naming the phase that builds them.
...
```bash
python -m venv .venv && .venv/Scripts/pip install -r ai-service/requirements.txt
```
...
npm --prefix server test        # 105 tests
```
...
The AI service's full requirements pull in torch via `sentence-transformers` — around two
gigabytes. To run only its test suite, `requirements-test.txt` is enough: the heavy imports are
lazy, and the tests inject fakes at the `Embedder` and `Generator` protocol seams.
